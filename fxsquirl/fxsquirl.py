...
class Chunker(analyzer.engine):
	...
	def __init__(self, doc, name: str = 'squirl', cfg: dict = {}, rst: int = 0):
	...
	def read(self, cfg, fx=None, tipe='load'):
		...
		while True:
			data = next(rdr,
			            None)  # progress this to utilizing the Sectn protocol with multiple sources and multiple lines of data per entry
	...
	def stop(self):
		'''Update the stop variable to stop the squirl processing service '''
		self.stop = True
		return self
	...
	def _processTree(self, srct, dataview, mdlt, pnode=''):
		...
		if srct == 'view':
			...
			cnt, ntree = 0, {}
			self.lock = True
			pout = _processOutline(tree)
			while self.lock:
				ntree = next(pout)
				yield ntree

	def _processOutline(self, tree):
		''' '''
		if len(tree.keys()) == 0:
			self.lock = False
			return
